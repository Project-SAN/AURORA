...
fn finalize(state: &mut State, ad_len_bytes: usize, msg_len_bytes: usize, tag_len: usize) -> Vec<u8> {
...
fn aes_round(input: Block, round_key: Block) -> Block {
    ...
    let shifted = [
        sub[0], sub[5], sub[10], sub[15], sub[4], sub[9], sub[14], sub[3], sub[8], sub[13],
        sub[2], sub[7], sub[12], sub[1], sub[6], sub[11],
    ];
...
mod tests {
    ...
    fn encrypt_vectors_match_spec() {
        for tv in positive_vectors() {
            ...
            let dec128 =
                decrypt_detached(&key, &nonce, &ad, &ct128, &tag128).expect("decrypt 128");
            assert_eq!(dec128, msg);

            let dec256 =
                decrypt_detached(&key, &nonce, &ad, &ct256, &tag256).expect("decrypt 256");
