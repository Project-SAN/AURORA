...
## Current gaps
- `router/src/main.rs` treats incoming setup frames as plain `{Sv, Si?}` blobs (`handle_setup_packet`) and never invokes `setup::node_process_with_policy`, so policy TLVs carried in AHDR are ignored.
